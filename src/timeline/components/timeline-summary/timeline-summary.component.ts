...
export class TimelineSummaryComponent implements OnInit, AfterViewInit {
        ...
        this._timelineService.timelineLocations.subscribe((locations: Array<TimelineEntry>) => {
            ...
            for (let i = 0; i < locations.length; i++) {
                this.timeEntries.push({
                    hours:
                        locations[i].timeA.getHours() >= 12
                            ? locations[i].timeA.getHours() - 12
                            : locations[i].timeA.getHours(),
    ...
    public updateTime(index: number) {
        let timeString = `${this.timeEntries[index].hours}:${this.timeEntries[index].minutes} ${
            this.timeEntries[index].am ? 'AM' : 'PM'
        }`;
        let time = new Date(
            0,
            0,
            0,
            this.timeEntries[index].hours,
            this.timeEntries[index].minutes,
            0
        );
