...
export class TripRouteModeDirective {
        ...
        for (let mode of TripDiary.config.modes) {
            ...
            for (let subMode of mode.subModes) {
                ...
                if (subMode.name == modeName) {
                    ...
                    return Color(subMode.colour).darken(0.3).hex();
                }
            }
        }
    };
            ...
            if (evt != null) {
                ...
                if (mode.dataInputs != undefined) {
                    if (mode.dataInputs.length > 0) {
                        ...
                        this.routeModeController.showDialog(evt, mode,
                            mode.dataInputs,
                            mode.dialogTitle,
                            this.tripDiaryService.getActiveTripLeg().data, (result) => {
                                /* ask for passengers */
                                this.tripDiaryService.setTripMode(mode["name"], mode["category"], mode.allowAddWaypoints);
                                this.tripDiaryService.addTripLegExtraData(this.tripDiaryService.state.activeRouteIndex,
                                    this.tripDiaryService.getActiveTripRoute()._activeTripLegIndex, result);
                            });
                        // }, 100);
                    }
                }
    ...
    public link($scope: ng.IScope, element: ng.IAugmentedJQuery, attrs: ng.IAttributes, controllers: Array<ng.IController>): void {
        ...
        this._$scope.$on('waypointsChanged', (evt, args) => {
            ...
            this._tripDiaryService.setTriplegWaypoints(this._tripDiaryService.state.activeRouteIndex, this._tripDiaryService.getActiveTripRoute()._activeTripLegIndex,
                waypoints);
         ...
         *
         */
        this._$scope.$watch('foundRoutes', (routes, oldValue) => {
