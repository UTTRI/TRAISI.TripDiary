...
export class SurveyMapDirective {
     ...
     */
    constructor(private _$http: ng.IHttpService, private _$compile, private _$templateRequest, private _tripDiaryService, private _$rootScope: ng.IRootScopeService) {
    ...
    public static Factory() {
        ...
        var directive = ($http, $compile, $templateRequest, tripDiaryService, $rootScope) => {
    ...
    public setSummaryLinesOpacity(opacity: number) {
        for (var r in this._summaryLines) {
    ...
    private generateRouterOptions(tripLeg: TripLeg, startTime: Date) {
        ...
        var today = new Date();
        let adjustedStartTime = new Date(today.getFullYear(), today.getMonth(), today.getDate(), startTime.getHours(), startTime.getMinutes());
    ...
    public addRouting(tripRoute: TripRoute, waypointChangedTrigger = false) {
        ...
        for (let key in this._layersByRoute) {

            for (let layer of this._layersByRoute[key]) {
        ...
        if (!this._routers.hasOwnProperty(tripRoute.id) || true) {
            ...
            if (tripRoute.editComplete == false || legActive == true) {
                ...
                if (activeTripLeg._mode == undefined) {
                    ...
                    this.addTripLegSummaryLineMarkers(tripRoute, activeTripLeg, _.findIndex(tripRoute.tripLegs, activeTripLeg), 0);
                ...
                for (let tripLeg of tripRoute.tripLegs) {
                    ...
                    if (tripLeg._mode == undefined) {
                        ...
                        this.addTripLegSummaryLineMarkers(tripRoute, tripLeg, _.findIndex(tripRoute.tripLegs, tripLeg), markerIndex);
                        continue;
                    }
                    var google_router = this.generateRouterOptions(tripLeg, tripRoute.startLocation.endTime);


                    let latLngs = [];

                    for (var i = 0; i < tripLeg._waypoints.length; i++) {
                            ...
                            let control = L.Routing.control({
                                ...
                                plan: L.Routing.plan(latLngs, {
                                    createMarker: (i, wp, n) => {
                                        ...
                                        if (i != 0 && i != n - 1) {
                                            var link = $('<span class="delete-popup">Delete</span>').on('click', (evt) => {
                                            ...
                                            this._$templateRequest(this.deletePopupTemplateUrl).then((template) => {
                                                ...
                                                let toolTip = this._$compile($('<span></span>').html(template).contents())(this._$scope);
                ...
                _.delay(() => {
                    ...
                    for (let tripLeg2 of tripRoute.tripLegs) {
                        ...
                        this.addTripLegSummaryLineMarkers(tripRoute, tripLeg2, _.findIndex(tripRoute.tripLegs, tripLeg2));
    ...
    private onRouteChanged = (route: TripRoute) => {
        ...
        console.log("inside of on route chnaged");
    ...
    private addDeleteModeSwitchPopup(marker, tripLegIndex) {
        ...
        this._$scope['deleteModeSwitch'] = ($event) => {
            ...
            this._tripDiaryService.removeModeSwitch(
                this._tripDiaryService.state.activeRouteIndex,
                $event.target['tripLegIndex']);
        }
    ...
    private addTripLegSummaryLineMarkers(tripRoute: TripRoute, tripLeg2: TripLeg, tripLegIndex: number, markerIndex: number = 0) {
        ...
        if (tripRoute.tripLegs.length == 1) {
            ...
            $(marker._icon).find("mark").css({display: "block"});
            $(marker._icon).find("mark").html("" + markerIndex);
        ...
        for (let tripLeg of tripRoute.tripLegs) {
            ...
            for (let i = 1; i < tripLeg.waypoints.length - 1; i++) {
                ...
                if (!tripLeg._isComplete) {
                    ...
                    this._layersByRoute[tripRoute.id].push(marker);
                    $(marker._icon).find("mark").css({display: "block"});
                    $(marker._icon).find("svg,i,img").remove();
                    $(marker._icon).find("mark").html("" + String.fromCharCode(97 + waypointIndex).toUpperCase());
