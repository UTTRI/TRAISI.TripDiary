...
export class TripDiaryTourService {
	...
	public timelineShown() {
		if (window.matchMedia('(min-width: 480px)').matches) {
			...
			if (!isNullOrUndefined(this.state().startLocation) && !isNullOrUndefined(this.state().endLocation) && !this._isTimelineHidden) {
				if (
					(this.state().tripRoutes.length == 0 && this.state().previousAction != ADD_TRIP_LOCATION_DATA) ||
	...
	public setElementScrollVisibility(elementName: string, visible: boolean) {
		...
		if (
			(elementName == ADD_START_LOCATION_BUTTON ||
				elementName == END_LOCATION_BUTTON ||
				elementName == ADD_INTERMEDIATE_LOCATION_BUTTON) &&
			visible
		) {
			this.timelineShown();
	...
	private ReplaceIterate(obj, regex, replace) {
		for (var property in obj) {
			if (obj.hasOwnProperty(property)) {
				if (typeof obj[property] == 'object') this.ReplaceIterate(obj[property], regex, replace);
