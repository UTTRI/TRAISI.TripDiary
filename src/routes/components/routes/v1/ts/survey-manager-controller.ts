...
export default class SurveyManagerController {
    ...
    private insertQuestions(questionId, data) {
        ...
        for (let p of this.pagesQuestions) {
            ...
            for (let q of Object.keys(p['questions'])) {
                ...
                if (parseInt(questionId.split("_")[1]) == parseInt(q)) {
    ...
    private submitForm() {
        $('form#survey_form').submit((e) => {
            $(this).find('.next-container').addClass('hidden');
        });
        //submit called
    ...
    private submit() {
        ...
        if (window.matchMedia('(min-width: 480px)').matches) {
            ...
            var question = this.questions[this.activePageNumber];
            var tripsInitialSearch = question.find('.trips-default-panel');
            var tripsInitialAndVisible = tripsInitialSearch.length > 0 && !tripsInitialSearch.hasClass('mobile-child-hide');
            var timelineAndVisible = question.hasClass('timeline-outer-container');
            var tripsDetailsAndVisible = question.hasClass('router-outer-container');
            var tripsDetailsAndVisible = question.hasClass('router-outer-container');
            if (question.find('.optional-question').length > 0 || question.find('.question-container').hasClass('success') || question.find('.household-question').length > 0 || timelineAndVisible || tripsDetailsAndVisible) {
                this.showNextSection();
            }
            else {
                if (tripsInitialAndVisible) {
                    displaySnackBar("Please Select an Option!", "errorMsg", 1500);
                    question.find('.question-container').first().addClass('error');
                }
    ...
    private previousPage() {
        ...
        let regex = /.+\/(\d)\//g;
        let pageNumber: number = parseInt(regex.exec(locationHref)[1]) - 1;
